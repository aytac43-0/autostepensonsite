  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            full_name: fullName, // Critical for profile trigger
          },
        },
      })
      ...
      // 2. Check if a user was actually created (or if it requires email confirmation)
      if (data.user) {
        toast({
        ...
        // Auto sign-in generally happens, but we can redirect to dashboard
        router.push('/dashboard')
        router.refresh()
      } else if (data.session === null && !error) {
        // Email confirmation scenario (if enabled in Supabase)
        toast({
